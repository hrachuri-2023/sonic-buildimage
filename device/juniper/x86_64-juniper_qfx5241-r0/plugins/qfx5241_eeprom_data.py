#!/usr/bin/env python3
#
# Name: juniper_qfx5241_eepromconv.py version: 1.0
#
...
def main():
    ...
    eeprom_file.write("===============================\r\n")
    eeprom_file.write("Product Name=%s\r\n" % eeprom_qfx5240.modelstr())
    eeprom_file.write("Part Number=%s\r\n" % eeprom_qfx5240.part_number_str())
    eeprom_file.write("Serial Number=%s\r\n" % eeprom_qfx5240.serial_number_str())
    eeprom_file.write("MAC Address=%s\r\n" % "00:a0:c9:00:00:00") #eeprom_qfx5240.base_mac_address()
    eeprom_file.write("Manufacture Date=%s\r\n" % eeprom_qfx5240.manuDate_str())
    eeprom_file.write("Platform Name=%s\r\n" % eeprom_qfx5240.platform_str())
    eeprom_file.write("Number of MAC Addresses=%s\r\n" % eeprom_qfx5240.MACsize_str())
    eeprom_file.write("Vendor Name=%s\r\n" % eeprom_qfx5240.vendor_name_str())
    eeprom_file.write("Manufacture Name=%s\r\n" % eeprom_qfx5240.manufacture_name_str())

    eeprom_dict = eeprom_qfx5240.system_eeprom_info()
    key = '0x29'
    ...
    vendor_value_formatted, vendor_value_hexvalue = 0, bytes.fromhex("4a756e69706572204e6574776f726b73") # eeprom_qfx5240.vendor_ext_str()

    eeprom_hex = '/etc/init.d/eeprom_qfx5241_hex'
    ...
    with open(eeprom_hex, 'rb+') as AsmID_Hexfile:
        ...
        vendorext = ""
        vendorext += "0x" + str(vendorext_read[0:2])
        for i in range(2, 58, 2):
            vendorext += " 0x" + str(vendorext_read[i:i+2])
    ...
    out_file = '/etc/init.d/MainEeprom_qfx5241_ascii'
    try:
        with open(out_file, 'w') as file:
    ...
    maineeprom_ascii = '/etc/init.d/MainEeprom_qfx5241_ascii'
    ...
    eeprom_hex = '/etc/init.d/MainEeprom_qfx5241_hex'
    # Write contents of Main Board EEPROM to new file in hexa format
    with open(eeprom_hex, 'wb+') as Hexfile:
...
class Eeprom(eeprom_tlvinfo.TlvInfoDecoder):
    def __init__(self):
        self.__eeprom_path = "/var/tmp/eeprom_24c02.bin"
