#!/usr/bin/env python
#
# Description: This file contains the Juniper QFX5241 Platform Initialization routines
#
...
'modprobe i2c-mux',
'modprobe mfd-core',
'modprobe qfx5241-jnx-garnet-core',
'modprobe qfx5241-i2c-ocores-garnet',
'modprobe qfx5241-gpio-garnet',
'modprobe qfx5241-jnx-xrefpga-lpcm',
]
...
def main():
    ...
    EepromDumpFile = '/var/tmp/eeprom.hex'
    I2cdumpCmd = ['i2cdump', '-y', '-f', '-r', '0-255', '1', '0x51', 'b']

    with open(EepromDumpFile, 'w') as outfile:
        subprocess.run(I2cdumpCmd, stdout=outfile, stderr=subprocess.DEVNULL)

    memory = bytearray([0xFF] * 256)  # Initialize with 0xFF (common erased EEPROM value)
    with open(EepromDumpFile, 'r') as f:
        for line in f:
            line = line.strip()
            if not line or ':' not in line:
                continue

            # Split line into address and data parts
            ...
            # Extract hex byte strings (ignore trailing ASCII column)
            bytes_strs = data_str.strip().split()
            for i, byte_str in enumerate(bytes_strs):
                if base_addr + i < 256:
                    try:
                        memory[base_addr + i] = int(byte_str, 16)
                    except ValueError:
                        # Ignore non-hex bytes (e.g., ASCII column or formatting)
                        pass

    with open('/var/tmp/eeprom_24c02.bin', 'wb') as f:
        f.write(memory)
    ...
    try:
        ...
        print(f'Error: Execution of {CreateEEPROMdeviceCmd} failed')
        # return False

    time.sleep(1)

    #Retrieve the Base MAC Address from EEPROM    
    status, macAddress = 0, "00:a0:c9:00:00:00" #getstatusoutput_noshell(["decode-syseeprom", "-m", "0x24"])
    if status:
    ...
    # Juniper QFX5241 platform drivers install
    do_install()
    time.sleep(2)

    # Juniper SFP Intialization    
    #JuniperSFPInitCmd = ['python', '/usr/share/sonic/device/x86_64-juniper_qfx5241-r0/plugins/qfx5241_sfp_init.py']
    #try:
    ...
    EEPROMDataCmd = ['python', '/usr/share/sonic/device/x86_64-juniper_qfx5241-r0/plugins/qfx5241_eeprom_data.py']
    try:
